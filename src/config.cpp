...
Result update(Config &config) {
    Result rc = 0;
    if (R_FAILED(rc = fizeauGetIsActive(&config.active))) return rc;
    if (R_FAILED(rc = fizeauGetActiveInternalProfileId(&config.active_internal_profile))) return rc;
    if (R_FAILED(rc = fizeauGetActiveExternalProfileId(&config.active_external_profile))) return rc;

    if (R_FAILED(rc = fizeauProfileGetDuskTime(&config.cur_profile, &config.dusk_begin, &config.dusk_end))) return rc;
    if (R_FAILED(rc = fizeauProfileGetDawnTime(&config.cur_profile, &config.dawn_begin, &config.dawn_end))) return rc;
    if (R_FAILED(rc = fizeauProfileGetCmuTemperature(&config.cur_profile, &config.temperature_day, &config.temperature_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetCmuColorFilter(&config.cur_profile, &config.filter_day, &config.filter_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetCmuGamma(&config.cur_profile, &config.gamma_day, &config.gamma_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetCmuSaturation(&config.cur_profile, &config.sat_day, &config.sat_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetCmuLuminance(&config.cur_profile, &config.luminance_day, &config.luminance_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetCmuColorRange(&config.cur_profile, &config.range_day, &config.range_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetScreenBrightness(&config.cur_profile, &config.brightness_day, &config.brightness_night))) return rc;
    if (R_FAILED(rc = fizeauProfileGetDimmingTimeout(&config.cur_profile, &config.dimming_timeout))) return rc;
...
Result apply(cfg::Config &config) {
    Result rc = 0;
    if (R_FAILED(rc = fizeauProfileSetDuskTime(&config.cur_profile, config.dusk_begin, config.dusk_end))) return rc;
    if (R_FAILED(rc = fizeauProfileSetDawnTime(&config.cur_profile, config.dawn_begin, config.dawn_end))) return rc;
    if (R_FAILED(rc = fizeauProfileSetCmuTemperature(&config.cur_profile, config.temperature_day, config.temperature_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetCmuColorFilter(&config.cur_profile, config.filter_day, config.filter_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetCmuGamma(&config.cur_profile, config.gamma_day, config.gamma_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetCmuSaturation(&config.cur_profile, config.sat_day, config.sat_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetCmuLuminance(&config.cur_profile, config.luminance_day, config.luminance_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetCmuColorRange(&config.cur_profile, config.range_day, config.range_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetScreenBrightness(&config.cur_profile, config.brightness_day, config.brightness_night))) return rc;
    if (R_FAILED(rc = fizeauProfileSetDimmingTimeout(&config.cur_profile, config.dimming_timeout))) return rc;
...
Result open_profile(Config &config, FizeauProfileId id) {
    ...
    if (R_FAILED(rc = fizeauOpenProfile(&config.cur_profile, id))) return rc;
