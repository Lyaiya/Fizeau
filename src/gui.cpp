...
tsl::elm::Element *ErrorGui::createUI() {
    ...
    auto *drawer = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
        renderer->drawString(format_string("%#x (%04d-%04d)", this->rc, R_MODULE(this->rc) + 2000, R_DESCRIPTION(this->rc)).c_str(),
    ...
    std::string jsonStr = R"(
        {
            ...
            "BrightnessFizeauOverlayGuiCategoryHeaderText": "Brightness: %.2f",
...
tsl::elm::Element *FizeauOverlayGui::createUI() {
    this->info_header = new tsl::elm::CustomDrawer([this](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
        renderer->drawString(format_string("EditProfileFizeauOverlayGuiCustomDrawerText"_tr, static_cast<std::uint32_t>(this->config->cur_profile_id) + 1).c_str(),
            false, x, y + 20, 20, renderer->a(0xffff));
        renderer->drawString(format_string("ProfileInPeriodFizeauOverlayGuiCustomDrawerText"_tr, this->is_day ? "ProfileInPeriodDayFizeauOverlayGuiCustomDrawerText"_tr.c_str() : "ProfileInPeriodNightFizeauOverlayGuiCustomDrawerText"_tr.c_str()).c_str(),
    ...
    this->range_button->setValue(is_full(this->is_day ? this->config->range_day : this->config->range_night) ? "ColorRangeFullFizeauOverlayGuiListItemText"_tr : "ColorRangeLimitedFizeauOverlayGuiListItemText"_tr);
...
void FizeauOverlayGui::update() {
    ...
    this->temp_header->setText(format_string("TemperatureFizeauOverlayGuiCategoryHeaderText"_tr,
        this->is_day ? this->config->temperature_day : this->config->temperature_night));
    this->brightness_header->setText(format_string("BrightnessFizeauOverlayGuiCategoryHeaderText"_tr,
        this->is_day ? this->config->brightness_day  : this->config->brightness_night));
    this->gamma_header->setText(format_string("GammaFizeauOverlayGuiCategoryHeaderText"_tr,
        this->is_day ? this->config->gamma_day       : this->config->gamma_night));
    this->sat_header->setText(format_string("SaturationFizeauOverlayGuiCategoryHeaderText"_tr,
        this->is_day ? this->config->sat_day         : this->config->sat_night));
    this->luma_header->setText(format_string("LuminanceFizeauOverlayGuiCategoryHeaderText"_tr,
        this->is_day ? this->config->luminance_day   : this->config->luminance_night));
}
